...
public class PlayOutTestCodeGen implements CodeGenRecorder.CodeGen {

	private final String withTests = "HelpWithTests";
	...
	@Override public String readOut(ImmutableScotlandYardView seed,
		...
		ls.add(format("TestHarness %s = new TestHarness();", withTests));

		List<ImmutableScotlandYardView.ImmutablePlayer> players = seed.players.stream()
				.map(v -> new ImmutableScotlandYardView.ImmutablePlayer(v.colour, configs.get(v.colour).location, v.tickets))
				.collect(toList());

		ls.addAll(players.stream().map(p -> format("PlayerConfiguration %s = %s.newPlayer(\n%s);",
				p.colour.name().toLowerCase(), withTests, mkPlayer(p))).collect(toList()));
		...
		ls.add(format("Spectator %s = %s.createSpectator();", spectatorV, withTests));
		...
		ls.add(format("%s.play(%s).startRotationAndAssertTheseInteractionsOccurInOrder(\n%s)" +
				...
				withTests, gameV, interactions.stream().collect(joining(", \n"))));
	...
	private static String mkPlayer(ImmutableScotlandYardView.ImmutablePlayer p) {
		String tickets = p.tickets().entrySet()
				.stream()
				.sorted(Entry.comparingByKey(comparingInt(ImmutableScotlandYardView.TicketAbbr::ordinal)))
	...
	private static String mkView(String seed,
		...
		delta.append(now.players.stream()
				.map(PlayOutTestCodeGen::mkPlayer)
				.collect(joining(", \n")));
		delta.append(")\n\t//</editor-fold>\n");
